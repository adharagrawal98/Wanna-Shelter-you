...
// Helper function to fetch PayPal email dynamically using uid
const getPayPalEmailByUid = async (uid) => {
    try {
        const docRef = db.collection("charityDetails").doc(uid); // Use uid as the document ID
        ...
        return charityData.PayPalEmail; // Ensure the Firestore document contains this field
    } catch (error) {
...
exports.createOrder = async (amount, currency = 'GBP', uid) => {
    try {
        // Fetch the PayPal email address using uid
            ...
            purchase_units: [
                {
                    ...
                    payee: {
                        email_address: payeeEmail, // Dynamic email from Firestore
                    },
                },
            ],
            application_context: {
